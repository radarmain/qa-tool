...
import { GAradarTableDataType, GAimpOtherTableDataType } from './types.tsx';
import './index.css'
...
import Ga_imp from './Ga_imp.tsx';
import Ga_radar from './Ga_radar.tsx';
...
const { Option } = Select;
  ...
  function Ga({ accountNamesGA }){
    ...
    const [table1Data, setTable1Data] = useState<Array<GAradarTableDataType>>([]);
    const [table2Data, setTable2Data] = useState<Array<GAimpOtherTableDataType>>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/fetch-ga-data', {
          params: {
    ...
    const sumSpendTable1 = table1Data.reduce((totalSpend, item) => totalSpend + item.cost_micros, 0);
    const sumSpendTable2 = table2Data.reduce((totalSpend, item) => totalSpend + item.cost, 0);

    // Calculate the sum of imps in table2Data
    const sumImpsTable1 = table1Data.reduce((totalImps, item) => totalImps + item.impressions, 0);
    const sumImpsTable2 = table2Data.reduce((totalImps, item) => totalImps + item.impressions, 0);
    const { accountNameGA } = useParams();
    console.log('accountNamesGA:', accountNameGA);

    useEffect(() => {
        if (accountNameGA) {
          setSelectedAccount(accountNameGA);
        }
      }, [accountNameGA]);
        ...
        <div className='pickers'>
          ...
          {/* {isLoading ? (
              <p>Loading account names...</p>
                ...
                <AccountSelect
                accountNames= {accountNamesGA.map(item => item.accountName)}
              ...
              />
                  ...
                  <div style={{ display: 'flex', marginLeft: '30px' }}>
                    <div style={{ flex: 1, marginRight: '30px' }}>
                      <h3>Radar Data</h3>
                      <Ga_radar tableData={table1Data} />
                    </div>
                    <div style={{ flex: 1, marginLeft: '50px' }}>
                      <h3>Improvado Data</h3>
                      <Ga_imp tableData={table2Data} />
                    </div>
                  </div>
                </div>
              )}
